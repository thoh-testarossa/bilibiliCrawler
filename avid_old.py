...
def GetInfo(avid=None):
    ...
    try:
        ...
        mid = page.select('a[class=up-detail]')[0].attrs['href'].replace('http://space.bilibili.com/','').replace('//space.bilibili.com/', '').replace('#!/video','')
        mid = int(mid)
...
if __name__ == "__main__":
    ...
    for avid in range(avid_Q + 1, maxID):
